...
"""vapor_modern.py: Modern async version of Virtual Dreams Bot for Telegram. Generates Vaporwave music."""
...
async def vapor(query, context, request_id, chat_id):
    ...
    try:
        await context.bot.send_message(chat_id=chat_id, text=working_str)
    ...
    if Path(vapor_path).is_file():
        vapor_path = title + "_vapor.wav"
        try:
            await context.bot.send_audio(chat_id=chat_id, audio=open(vapor_path, 'rb'))
        except Exception as e:
            logger.error("[" + str(request_id) + "] " + str(e))
...
async def vapor_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        start = timer()
        await vapor(request_text, context, request_id, chat_id)
    except ValueError as e:
        logger.error("[" + str(request_id) + "] " + str(e))
        status = "failed"
        await context.bot.send_message(chat_id=chat_id, text=error_str)
    finally:
        end = timer()
...
async def unknown_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(unknown_str)
...
def main():
    ...
    application.add_handler(vapor_handler)
    ...
    application.run_webhook(
        listen="0.0.0.0",
        port=PORT,
        webhook_url="https://{}.herokuapp.com/{}".format(HEROKU_NAME, BOT_TOKEN),
        url_path=BOT_TOKEN
    )
